    ...
    for dir in ball_directions:
        for ch in change:
            for dir_ch in dir_change:
                ...
                trial_goal_size  = float(trial_goal_dis)*float(trial_ratio)
        ...
        for frameN in range(trialtime):
            ...
            if int(myball.pos[1]) == int(mygoal.start[1]):
                ...
                if keypress == 'v' and sign == '-' or keypress == 'n' and sign == '+':
                    ...
                    break
        break
    resp_data = pd.DataFrame(data = [[keypress,trial_id+1,t_start,t_set - t_start, t_ball - t_start, t_release, t_response - t_ball, t_goal-t_start, t_change-t_start]], columns = ['response','trial_nr','t_start','t_set','t_ball','t_release','t_response','t_goal','t_change'])
