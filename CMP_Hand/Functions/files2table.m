  ...
  if session == '10'
    ...
    for b = 1:length(data.block)
      ...
      for t = 1:length(b_data.trial)
        ...
        sessions = [sessions, str2num(session)];
        blocks   = [blocks, b];
        trials   = [trials, t];
        effector(l) = 'H' ;
        l = l+1;
        ...
        #xPos_start
        #yPos_start
        des_dat = sprintf('%s\t%i\t%i\t%i\t%s\t%i\t%i\t%i\t%i\t%i\t', participant, str2num(session), b, t, 'H', t_des.goalPos, t_des.jumpTim, t_des.jumpPos, t_des.fixT, ok_resT);
        # ONLINE DATA:
...
save('octave_data.txt', 'octave_data') 
